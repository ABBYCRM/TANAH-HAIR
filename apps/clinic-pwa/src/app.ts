...
const state: { user: User | null; csrfToken: string; route: string } = { user: null, csrfToken: '', route: location.hash.slice(1) || 'dashboard' };
...
async function visualizationView() {
  ...
  document.querySelector<HTMLFormElement>('#visual-form')?.addEventListener('submit', async event => {
    ...
    try {
      ...
      document.querySelector<HTMLElement>('#visual-output')!.innerHTML = `<img src="${result.outputDataUrl}" alt="Watermarked hypothetical synthetic hair concept"><p class="muted">${escapeHtml(result.model)} · ${escapeHtml(result.createdAt)}</p>`;
...
async function settingsView() {
  ...
  document.querySelector<HTMLButtonElement>('#test-gemini')!.addEventListener('click', async () => {
    ...
    try {
      ...
      toast(result.modelVisible ? 'Connected and selected model is visible.' : 'Connected, but selected model was not listed for this key.'); await settingsView();
    } catch (error) { toast((error as Error).message, 'error'); }
  });
}
...
async function render() {
  ...
  try {
    ...
    root.innerHTML = `<main class="fatal"><h1>Unable to load workspace</h1><p>${escapeHtml((error as Error).message)}</p><button onclick="location.reload()">Retry</button></main>`;
  }
}
