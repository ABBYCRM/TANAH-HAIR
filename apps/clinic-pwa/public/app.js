...
async function renderSimulatorView() {
    ...
    document.querySelector('#sim-ai-generate')?.addEventListener('click', async (event) => {
        const button = event.currentTarget;
        const renderBtn = document.querySelector('#sim-render');
        const variantsBtn = document.querySelector('#sim-variants');
        const multiBtn = document.querySelector('#sim-multi');
        const multiAiBtn = document.querySelector('#sim-ai-multi');
        ...
        try {
            const form = document.querySelector('#sim-form');
            const body = Object.fromEntries(new FormData(form));
            const result = await api('/simulator/ai-generate', { method: 'POST', body: JSON.stringify(body) });
            wrap.classList.remove('visual-placeholder');
            ...
            toast('AI generator unavailable — falling back to parametric render. ' + error.message, 'error');
    ...
    document.querySelector('#sim-ai-multi')?.addEventListener('click', async (event) => {
        const button = event.currentTarget;
        const renderBtn = document.querySelector('#sim-render');
        const variantsBtn = document.querySelector('#sim-variants');
        const multiBtn = document.querySelector('#sim-multi');
        const aiBtn = document.querySelector('#sim-ai-generate');
        ...
        try {
            const form = document.querySelector('#sim-form');
            const body = Object.fromEntries(new FormData(form));
            const result = await api('/simulator/ai-multi-view', { method: 'POST', body: JSON.stringify(body) });
            grid.innerHTML = result.views.map((v) => {
                if (v.error) {
            ...
            toast('AI 4-view failed: ' + error.message, 'error');
            grid.innerHTML = `<p class="muted">AI 4-view failed: ${escapeHtml(error.message)}</p>`;
        }
        finally {
